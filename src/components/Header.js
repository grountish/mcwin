    ...
    <div className="z-50 w-full bg-transparent absolute top-0 left-0 text-white">
      <div className="flex items-center justify-between lg:px-20 px-6 lg:py-14 py-10">
        ...
        <div className="hidden lg:flex uppercase">
          <ul className="hidden lg:flex lg:items-center lg:w-auto lg:space-x-6 !list-none">
            {header.map((link, i) => (
              <div key={i} className={path === link.slug ? "font-bold" : ""}>
      ...
      {/* 
          MOBILE
               */}
      <div
        className={`fixed top-0 bottom-0 left-0 px-9 py-3 transition-all duration-[450ms] overflow-y-auto transform bg-black opacity-90  w-full rounded shadow-md navbar-menu z-100 block lg:hidden overflow-x-hidden ${
